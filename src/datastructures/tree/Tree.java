...
public class Tree {
    ...
    private  Node createTreeFromPreInOrder(int[] preOrder,int[] inOrder, int left,int right){
        ...
        for(int i =0;i<inOrder.length;i++){
            if (preOrder[pIndex]==inOrder[i]){ current=i; break;}
        }
    ...
    private boolean isBST(Node node,int min , int max){
        ...
        if (Integer.parseInt(node.getValue())<= min || 
            Integer.parseInt(node.getValue()) > max) 
            return false;
    ...
    private void printTrasversal(Node root, String mode){
        if(root== null ) return;
        if("PRE".equals(mode)) System.out.print(" "+root.getValue()+" ");// Pre Orden
        printTrasversal(root.getLeft(),mode);
        if("IN".equals(mode)) System.out.print(" "+root.getValue()+" "); // In Orden
        printTrasversal(root.getRight(),mode);
        if("POS".equals(mode)) System.out.print(" "+root.getValue()+" "); // Post Orden
