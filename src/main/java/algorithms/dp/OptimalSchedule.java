...
/**
 * Problem Statement You're a delivery person, and you want to try planning out
 * your schedule. You can view a list of deliveries along with their associated
 * start time, end time, and dollar amount for completing the order. Assuming
 * dashers can only deliver one order at a time, determine the maximum amount of
 * money you can make from the given deliveries.
 * 
 ...
 * </ul>
 * 
 * The output should be an integer representing the maximum amount of money you
 * can make by forming a schedule with the given deliveries.
 * 
 * </p>
 * Constraints
 * <ul>
...
public class OptimalSchedule {
  ...
  public int maxProfit(int[] starts, int[] ends, int[] pays) {
    ...
    Arrays.sort(jobs, (Job a, Job b) -> a.start - b.start);
    // Recursion
  ...
  private int findMaxProfit(Job[] jobs, int index) {
    ...
    // job
    int max = Math.max(findMaxProfit(jobs, nextIndex) + jobs[index].pay, findMaxProfit(jobs, index + 1));
    memo.put(index, max);
    return max;
  }
